...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 2132, 3132));
...
static bool AppInitRPC(int argcx, char* argv[])
{
    //
    // Parameters
    //
...
int CommandLineRPC(int argcx, char* argv[])
{
    string strPrint;
    int nRet = 0;
    try {
        // Skip switches
        while (argcx > 1 && IsSwitchChar(argv[1][0])) {
            argcx--;
            argv++;
        }
        ...
        std::vector<std::string> strParams(&argv[2], &argv[argcx]);
