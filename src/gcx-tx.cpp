...
static bool AppInitRawTx(int argcx, char* argv[])
{
    //
    // Parameters
    //
...
static int CommandLineRawTx(int argcx, char* argv[])
{
    string strPrint;
    int nRet = 0;
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded gcx transaction
...
int main(int argcx, char* argv[])
{
    ...
    try {
        ret = CommandLineRawTx(argcx, argv);
    } catch (std::exception& e) {
