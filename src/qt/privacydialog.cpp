                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGCX ought to be enough for anybody." - Bill Gates, 2017
    ui->zGCXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGCXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGCX->setEnabled(false);
        ui->pushButtonMintzGCX->setToolTip(tr("zGCX is currently disabled due to maintenance."));

        ui->pushButtonSpendzGCX->setEnabled(false);
        ui->pushButtonSpendzGCX->setToolTip(tr("zGCX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGCXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGCX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGCX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGCX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGCX in ") + 
...
void PrivacyDialog::on_pushButtonSpendzGCX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGCX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGCX
        sendzGCX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGCX
    sendzGCX();
}

void PrivacyDialog::on_pushButtonZGCControl_clicked()
{
    ZGCControlDialog* zGCControl = new ZGCControlDialog(this);
    zGCControl->setModel(walletModel);
    zGCControl->exec();
}

void PrivacyDialog::setZGCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGCXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGCX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GCXX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGCXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGCX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GCX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGCX</b>";
    ...
    // use mints from zGCX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGCControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGCControlDialog::GetSelectedMints();
    }
    ...
    // Clear zGCX selector in case it was used
    ZGCControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGCX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GCX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGCX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGCX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGCX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGCX "));
    ui->labelzGCXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGCXSyncStatus->setVisible(fShow);
}
