...
#include "ui_zGCXcontroldialog.h"
...
std::list<std::string> ZGCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGCControlDialog::listMints;

ZGCControlDialog::ZGCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGCControlDialog::~ZGCControlDialog()
{
    delete ui;
}

void ZGCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGCControlDialog::updateList()
{
...
void ZGCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGCX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGCControlDialog::GetSelectedMints()
{
...
void ZGCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
