...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total gcx balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zGCXsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zGCX denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zGCX denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zGCX denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zGCX denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zGCX denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zGCX denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zGCX denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zGCX denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zGCX denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in gcx/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in gcx/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zGCXObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zGCXObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zGCXsupply", zGCXObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"gcxaddress\"\n"
            "\nReturn information about the given gcx address.\n"
            "\nArguments:\n"
            "1. \"gcxaddress\"     (string, required) The gcx address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"gcxaddress\", (string) The gcx address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: GCXX address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are gcx addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) gcx address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"gcxaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"gcxaddress\"  (string, required) The gcx address to use for the signature.\n"
